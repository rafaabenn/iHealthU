...
export class DashboardService {
  ...
  getToday(userId: string) {
    ...
    const userSleepArr = sleepArray(allSleep[userId]);
    const latestSleep  = userSleepArr[0] ?? null;
    ...
    return {
      ...
      workouts,
      currentStreak:  this.calculateGlobalStreak(userId),
    };
  }

  calculateGlobalStreak(userId: string): number {
    const allActivities: any[]          = readJSON(ACTIVITIES_PATH) ?? [];
    const allGoals: Record<string, any> = readJSON(GOALS_PATH)      ?? {};
    const allWater: Record<string, any> = readJSON(WATER_LOGS_PATH) ?? {};
    const allSleep: Record<string, any> = readJSON(SLEEP_LOGS_PATH) ?? {};
    ...
    const caloriesGoal      = Number(goals.dailyCalories)      || 0;

    const checkDay = (dateStr: string): boolean => {
      const dayActivities = activities.filter((a) => a.date?.startsWith(dateStr));
      const activeMinutes = dayActivities.reduce((s, a) => s + Number(a.duration || 0), 0);
      const calories      = dayActivities.reduce((s, a) => s + Number(a.calories  || 0), 0);
      const water         = Number(userWater[dateStr] || 0);
      const sleep         = userSleepArr.find((s: any) => s.date === dateStr)?.duration || 0;
  ...
  getSummary(userId: string) {
    ...
    const totalCalories = weekActivities.reduce((s: number, a: any) => s + Number(a.calories || 0), 0);
    const totalWorkouts = weekActivities.length;
    const totalDuration = weekActivities.reduce((s: number, a: any) => s + Number(a.duration || 0), 0);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayCals   = dayActivities.reduce((s: number, a: any) => s + Number(a.calories || 0), 0);
      const dayActive = dayActivities.reduce((s: number, a: any) => s + Number(a.duration || 0), 0);
      const dayWater  = Number(userWater[dateStr] || 0);
      const daySleep  = userSleepArr.find((s: any) => s.date === dateStr)?.duration || 0;

      dailyData.push({ day: dayName, dateStr, calories: dayCals, activeMinutes: dayActive, water: dayWater, sleep: daySleep });
    }
